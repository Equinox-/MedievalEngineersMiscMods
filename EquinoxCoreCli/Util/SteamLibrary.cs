        ...
        public static string FindAppInstallDir(uint appId)
        {
            ...
            while (appDirectories.Count > 0)
            {
                ...
                if (File.Exists(libraryAux))
                {
                    ...
                    if (!string.Equals(prop.Key, "LibraryFolders", StringComparison.OrdinalIgnoreCase))
                ...
                if (File.Exists(appAux))
                {
                    ...
                    if (!string.Equals(prop.Key, "AppState", StringComparison.OrdinalIgnoreCase))
                        throw new InvalidOperationException($"Root key on app manifest was incorrect ({prop.Key})");

                    var child = (VObject)prop.Value;
                    foreach (var kv in child.Properties())
                        if (string.Equals(kv.Key, "installdir", StringComparison.OrdinalIgnoreCase)
                            && kv.Value is VValue value && value.Value is string installDir)
                        {
