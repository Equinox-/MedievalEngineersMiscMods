...
namespace Equinox76561198048419394.Core.ModelCreator
{
    ...
    {
        private readonly DefinitionObSet SourceSet;
        private readonly DefinitionObSet DestinationSet;
        private readonly List<MethodInfo> Translators;
        private readonly List<MethodInfo> ConditionalTranslators;
        private readonly string SubtypeSuffix;
        private readonly Dictionary<MyDefinitionId, MyDefinitionId> TranslationTable;
            ...
            Dictionary<MyDefinitionId, MyDefinitionId> translationTable)
        {
            ...
            Translators = typeof(DefinitionObTranslator).GetMethods((BindingFlags) (-1)).Where(x => x.Name == nameof(TranslateInternal))
                .ToList();
            ConditionalTranslators = typeof(DefinitionObTranslator).GetMethods((BindingFlags) (-1)).Where(x => x.Name == nameof(ConditionalTranslateInternal))
                .ToList();
        ...
        {
            var original = SourceSet.GetDefinition<MyObjectBuilder_GeneratedBlockDefinition>(x.Id);
            if (original == null)
                return x;
            ...
            if (x.ReplaceItems != null)
            {
                ...
                {
                    var o = SourceSet.GetDefinition<MyObjectBuilder_GeneratedBlockDefinition>(k.To);
            ...
            MyObjectBuilder_CubeBlockDefinition.CbObCubeBlockComponent original)
        {
            ...
            var replacedComponent = TranslationTable.GetValueOrDefault(originalComponent, originalComponent);
            var replacedReturned = originalReturned.HasValue
                ? TranslationTable.GetValueOrDefault(originalReturned.Value, originalReturned.Value)
                : (MyDefinitionId?) null;
